...
object MockEvaDispatcher : Dispatcher() {

    val inputStream = Files.newInputStream(Paths.get("src/test/resources/bi1.xls"))
    val buffer = Buffer().readFrom(inputStream)

    override fun dispatch(request: RecordedRequest): MockResponse {
        return when (request.path) {
            "/stundenplan/anzeigen/?weeks=15%3B16%3B17%3B18%3B19%3B20%3B21%3B22%3B23%3B24%3B25%3B26%3B27%3B28&days=1-7&mode=xls&identifier_semester=BCS2&show_semester=&identifier_dozent=&identifier_raum=&term=1234" -> {
                MockResponse()
                    .setResponseCode(200)
                    .setBody(buffer)
            }
